...
exports.createReview = async (req, res) => {
  try {
    ...
    const errors = [];
    if (!rating || rating < 1 || rating > 5) errors.push("Please select a rating between 1 and 5.");
    if (!content) errors.push("Review content cannot be empty.");

    if (errors.length > 0) {
      return res.render("add-review", { recipeId, errors, user: req.user || null });
    }
...
exports.showEditReview = async (req, res) => {
  try {
    ...
    if (!req.user) return res.redirect("/login");
    if (!mongoose.Types.ObjectId.isValid(reviewId)) return res.send("Invalid review ID.");

    const review = await Review.findByIdReview(reviewId);
    if (!review) return res.send("Review not found.");

    if (review.userId.toString() !== req.user._id.toString()) return res.send("Not allowed.");

    const recipe = await Recipe.findByIdRecipe(review.recipeId);
    return res.render("edit-review", { review, recipe, errors: [], user: req.user || null });
  } catch (error) {
    console.error(error);
...
exports.updateReview = async (req, res) => {
  try {
    ...
    await Review.updateById(reviewId, { rating, content, updatedAt: new Date() });
